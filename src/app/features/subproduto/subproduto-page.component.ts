...
@Component({
  selector: 'app-subproduto-page',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  templateUrl: './subproduto-page.component.html',
})
export class SubProdutoPageComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.http.get<SubProduto[]>(this.apiUrl).subscribe({
      next: (data) => {
        this.items.set(data);
        this.loading.set(false);
      },
      error: () => {
        this.toast.error('Erro ao carregar SubProduto');
        this.loading.set(false);
      }
    });
  }
